...
#     y=torch.tensor(y, dtype=torch.float)
    ...
    loader = DataLoader(dataset, batch_size=len(X))
    return loader

def torch_binary_acc(y_pred, y_test):
...
def train(model, train_loader, test_loader, criterion, optimizer, iters=50000):
    ...
    for epoch in range(iters):        
        ...
        for output, target in train_loader:
        ...
        if epoch%100 == 0:
            with torch.no_grad():
                ...
                for X, Y in train_loader:
                    output, target = X.to(device), Y.to(device)
                    output = model(output)              
                    train_loss_ = criterion(output, target)
                    train_loss += train_loss_.item()
                train_costs.append(train_loss)
                train_acc = torch_binary_acc(output, target)
                ...
                for X, Y in test_loader:
                    output, target = X.to(device), Y.to(device)
                    output = model(output)              
                    test_loss_ = criterion(output, target)
                    test_loss += test_loss_.item()
                test_costs.append(test_loss)
                test_acc = torch_binary_acc(output, target)
...
def initialize_parameters(layer_dims):
    ...
    for l in range(1, L):
        W = np.random.randn(layer_dims[l], layer_dims[l-1]) / np.sqrt(layer_dims[l-1])
...
#         parameters['b' + str(l)] = torch.zeros(layer_dims[l], 1)
        parameters['W' + str(l)] = torch.tensor(W)
        parameters['b' + str(l)] = torch.tensor(b)
