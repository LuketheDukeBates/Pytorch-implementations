...
def initialize_parameters(n_x, n_h, n_y):
    ...
    W1 = torch.randn((n_h, n_x), dtype=float)*0.01
    b1 = torch.zeros(n_h, 1, dtype=float)
    W2 = torch.randn(n_y, n_h, dtype=float)*0.01
    b2 = torch.zeros(n_y, 1, dtype=float)
...
def initialize_parameters_deep(layer_dims):
    ...
    for l in range(1, L):
        denom = torch.sqrt(torch.tensor(layer_dims[l-1], dtype=torch.double))
        parameters['W' + str(l)] = torch.randn((layer_dims[l], layer_dims[l-1]), dtype=torch.double) / denom #*0.01
        parameters['b' + str(l)] = torch.zeros(layer_dims[l], 1, dtype=torch.double)
...
def compute_cost(AL, Y):
    ...
    cost = -torch.sum(torch.mul(Y, torch.log(AL)) + torch.mul(1-Y, torch.log(1-AL))) / m 
    ### END CODE HERE ###
    
    cost = cost.item()     # To make sure your cost's shape is what we expect (e.g. this turns [[17]] into 17).
...
def print_mislabeled_images(classes, X, y, p):
    ...
    for i in range(num_images):
        ...
        plt.axis('off')
        plt.title("Prediction: " + classes[int(p[0,index].item())].decode("utf-8") + " \n Class: " + classes[int(y[0,index].item())].decode("utf-8"))